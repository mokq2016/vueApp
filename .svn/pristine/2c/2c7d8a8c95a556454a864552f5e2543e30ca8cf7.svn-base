  ...
  methods: {
    init() {
      ...
      wsService.getSqxh().then(function(result) { //获取申请序号
        if (result.success) {
          ...
          //获取附报资料列表
          wsService.getFbzlList(self.sqxh, constant.swsxConstants.SWSX_DM_CWKJZDBA).then(
            function(result) {
              if (result.success) {
    ...
    saveFbzlxxWithoutImageData(fileArr, params) {
      ...
      let param = {
        ...
        swsxdm: constant.swsxConstants.SWSX_DM_CWKJZDBA,

...
export default {
      ...
      this.$router.push({
        ...
        query: {
         ...
         /* this.hbszData = JSON.parse(this.$route.query.hbszData);*/
          dzfpzgkpxeData:JSON.stringify(this.dzfpzgkpxeData),
          jldwData:JSON.stringify(this.jldwData),
          fpgmfsData:JSON.stringify(this.fpgmfsData),
          opType: 'modify',
          index: index
        }
      });
  ...
  activated() {
    if (this.$route.meta['needFresh']) {
      ...
      if (this.$route.meta['fromPath'] == '/modifyFppzhd') { //从添加过来时刷新列表
        this.yhxxData = this.fppzhdtzData;
      }
    }
  }
}
